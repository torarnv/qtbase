...
/*!
    ...
    The code originates from \c qwindowskeymapper.cpp.
*/

QWinRTKeyMapper::QWinRTKeyMapper()
    : m_keyGrabber(0)
{
...
// NOTE: The order of these states match the order in QWinRTKeyMapper::updatePossibleKeyCodes()!
...
*/
inline int fallbackQtKey(int keyCode)
{
...
void QWinRTKeyMapper::deleteLayouts()
{
    for (size_t i = 0; i < NumKeyboardLayoutItems; ++i)
        keyLayout[i].exists = false;
}

void QWinRTKeyMapper::clearRecordedKeys()
{
...
void QWinRTKeyMapper::updateKeyMap(ABI::Windows::UI::Core::IKeyEventArgs *args)
{
    VirtualKey vkey;
    args->get_VirtualKey(&vkey);
    CorePhysicalKeyStatus status;
    args->get_KeyStatus(&status);
    m_keyboardBuffer[vkey] = (status.IsKeyReleased ? 0 : 0x80);
    updatePossibleKeyCodes(m_keyboardBuffer, status.ScanCode, vkey);
}

void QWinRTKeyMapper::updatePossibleKeyCodes(unsigned char *kbdBuffer, quint32 scancode,
                                               quint32 vk_key)
{
    Q_UNUSED(scancode)
    ...
    // TODO: properly fill qtKey[0-7] as soon as we found a way, how to get these
    int fallbackKey = fallbackQtKey(vk_key);
...
bool QWinRTKeyMapper::isADeadKey(unsigned int vk_key, unsigned int modifiers)
{
...
bool QWinRTKeyMapper::translateKeyEvent(QWindow *widget, ABI::Windows::UI::Core::IKeyEventArgs *args)
{
    updateKeyMap(args);
    return translateKeyEventInternal(widget, args, false);
}

bool QWinRTKeyMapper::translateKeyEventInternal(QWindow *window, ABI::Windows::UI::Core::IKeyEventArgs *args, bool /* grab */)
{
    CorePhysicalKeyStatus status;
    args->get_KeyStatus(&status);
    VirtualKey vkey;
    args->get_VirtualKey(&vkey);
    const quint32 scancode = status.ScanCode;
    const quint32 vk_key = vkey;
    const bool isNumpad = (vk_key >= 96 && vk_key <= 111);
    quint32 nModifiers = 0;

    QWindow *receiver = m_keyGrabber ? m_keyGrabber : window;

    // Map native modifiers to some bit representation
    nModifiers |= (m_keyboardBuffer[VirtualKey_LeftShift    ] & 0x80 ? ShiftLeft : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_RightShift   ] & 0x80 ? ShiftRight : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_Shift        ] & 0x80 ? ShiftAny : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_LeftControl  ] & 0x80 ? ControlLeft : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_RightControl ] & 0x80 ? ControlRight : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_Control      ] & 0x80 ? ControlAny : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_LeftMenu     ] & 0x80 ? AltLeft : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_RightMenu    ] & 0x80 ? AltRight : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_Menu         ] & 0x80 ? AltAny : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_LeftWindows  ] & 0x80 ? MetaLeft : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_RightWindows ] & 0x80 ? MetaRight : 0);
    // Add Lock keys to the same bits
    nModifiers |= (m_keyboardBuffer[VirtualKey_CapitalLock  ] & 0x01 ? CapsLock : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_NumberKeyLock] & 0x01 ? NumLock : 0);
    nModifiers |= (m_keyboardBuffer[VirtualKey_Scroll       ] & 0x01 ? ScrollLock : 0);
    ...
    const bool isDeadKey = isADeadKey(vk_key, state);
    bool result = false;
    ...
    int code = 0;
    if (isNumpad && (state & Qt::AltModifier))
        code = fallbackQtKey(vk_key);
    else if (!isDeadKey && code < 0x20 || code == 0x7f)
        code = fallbackQtKey(vk_key);
    ...
    if (!(status.IsExtendedKey)) {
        ...
        case VirtualKey_NumberPad9:
            state |= (vk_key >= 96 && vk_key <= 111)
                    ? 0 : int(Qt::KeypadModifier);
        default:
            if (scancode == 0x004c0001 || scancode == 0xc04c0001)
    ...
    if (!status.IsKeyReleased) {
        ...
        KeyRecord *rec = key_recorder.findKey(vk_key, false);
        ...
        if (rec && rec->state != state) {
            key_recorder.findKey(vk_key, true);
            rec = 0;
        }

        QChar uch;
        if (vk_key == VirtualKey_Delete)
            uch = QChar(QLatin1Char(0x7f)); // Windows doesn't know this one.
        else
            uch = QChar((ushort)vk_key);
        if (uch.isLetter() && ((scancode & KF_ALTDOWN) || !(state & Qt::ShiftModifier)))
        ...
        if (rec) {
            if (code < Qt::Key_Shift || code > Qt::Key_ScrollLock) {
                QWindowSystemInterface::handleExtendedKeyEvent(receiver, QEvent::KeyRelease, code,
                                                               Qt::KeyboardModifier(state), scancode, vk_key, nModifiers, rec->text, true, 0);
        ...
        // and store the key data into our records.
            ...
            key_recorder.storeKey(vk_key, a, state, text);
            QWindowSystemInterface::handleExtendedKeyEvent(receiver, QEvent::KeyPress, code,
                                                           Qt::KeyboardModifier(state), scancode, vk_key, nModifiers, text, false, 0);
            result =true;
            bool store = true;
            if (!store)
                key_recorder.findKey(vk_key, true);
        ...
        KeyRecord* rec = key_recorder.findKey(vk_key, true);
        if (!rec && !(code == Qt::Key_Shift
            ...
            if (!code)
                code = asciiToKeycode(rec->ascii ? rec->ascii : vk_key, state);

            // Map SHIFT + Tab to SHIFT + BackTab, QShortcutMap knows about this translation
            if (code == Qt::Key_Tab && (state & Qt::ShiftModifier) == Qt::ShiftModifier)
                code = Qt::Key_Backtab;
            QWindowSystemInterface::handleExtendedKeyEvent(receiver, QEvent::KeyRelease, code,
                                                           Qt::KeyboardModifier(state), scancode, vk_key, nModifiers,
...
Qt::KeyboardModifiers QWinRTKeyMapper::queryKeyboardModifiers()
{
    Qt::KeyboardModifiers modifiers = Qt::NoModifier;
    if (m_keyboardBuffer[VirtualKey_LeftShift] != 0
            || m_keyboardBuffer[VirtualKey_RightShift] != 0
            || m_keyboardBuffer[VirtualKey_Shift] != 0)
        modifiers |= Qt::ShiftModifier;
    if (m_keyboardBuffer[VirtualKey_LeftControl] != 0
            || m_keyboardBuffer[VirtualKey_RightControl] != 0
            || m_keyboardBuffer[VirtualKey_Control] != 0)
        modifiers |= Qt::ControlModifier;
    if (m_keyboardBuffer[VirtualKey_LeftMenu] != 0
            || m_keyboardBuffer[VirtualKey_RightMenu] != 0
            || m_keyboardBuffer[VirtualKey_Menu] != 0)
